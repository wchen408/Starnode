...
class StarNode:
	...
	def selectHubnCheck(self):
		...
		self.logger.debug("Heartbeat from %s:\
		\n				# of active peer(s) in the network: %d \
		\n				peers: %s \
		\n 				Hub: {%s}", \
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Starnode Network')
	parser.add_argument('node_name', type=str,\
	 help='name of the Starnode instance')
	parser.add_argument('local_port', type=int, default=8888,\
		help='port the Starnode instance listens to')
	parser.add_argument('-pa', '--PoC_address', type=str, nargs='?', default=None,\
		help='hostname or ip of PoC')
	parser.add_argument('-pc', '--PoC_port', type=int, nargs='?', default=None,\
		help='port of PoC')
	parser.add_argument('MaxNode', type=int, default=10,\
		help='max capacity of StarNode network')
